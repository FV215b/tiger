...
datatype svalue = VOID | ntVOID of unit ->  unit
 ...
 | recordargs of unit ->  ( ( A.symbol * A.exp * A.pos )  list)
 | explist of unit ->  (A.exp list) | optionalexp of unit ->  (A.exp)
...
|  ( 3, ( ( _, ( MlyValue.dec dec1, _, dec1right)) :: ( _, ( 
...
|  ( 4, ( ( _, ( MlyValue.tydec tydec1, tydec1left, tydec1right)) :: 
...
|  ( 5, ( ( _, ( MlyValue.vardec vardec1, vardec1left, vardec1right))
...
|  ( 6, ( ( _, ( MlyValue.fundec fundec1, fundec1left, fundec1right))
 :: rest671)) => let val  result = MlyValue.dec (fn _ => let val  (
...
|  ( 11, ( ( _, ( MlyValue.ID ID2, _, ID2right)) :: ( _, ( _, _, 
...
MlyValue.tyfields (fn _ => let val  ID1 = ID1 ()
 ...
 in (
{ name = Symbol.symbol ID1, typ = Symbol.symbol ID2, escape = ref true, pos = COLONright } :: tyfields 
)
end)
...
MlyValue.vardec (fn _ => let val  ID1 = ID1 ()
 ...
 val  (exp as exp1) = exp1 ()
 in (
A.VarDec { name = Symbol.symbol ID1, typ = SOME (Symbol.symbol ID2, ID2left), init = exp, pos = VARright, escape = ref true }
)
end)
 in ( LrTable.NT 6, ( result, VAR1left, exp1right), rest671)
end
...
) => let val  result = MlyValue.optionalexp (fn _ => let val  (exp as 
exp1) = exp1 ()
 in (exp)
end)
...
|  ( 19, ( ( _, ( MlyValue.ntVOID empty1, empty1left, empty1right)) ::
 rest671)) => let val  result = MlyValue.optionalexp (fn _ => let val 
 empty1 = empty1 ()
 in (A.NilExp)
end)
 in ( LrTable.NT 9, ( result, empty1left, empty1right), rest671)
end
|  ( 20, ( ( _, ( MlyValue.ntVOID empty1, empty1left, empty1right)) ::
...
|  ( 23, ( ( _, ( MlyValue.ntVOID empty1, empty1left, empty1right)) ::
 rest671)) => let val  result = MlyValue.recordargs (fn _ => let val  
empty1 = empty1 ()
 in (nil)
end)
 in ( LrTable.NT 11, ( result, empty1left, empty1right), rest671)
end
|  ( 24, ( ( _, ( MlyValue.exp exp1, _, exp1right)) :: ( _, ( _, _, 
EQright)) :: ( _, ( MlyValue.ID ID1, ID1left, _)) :: rest671)) => let
 val  result = MlyValue.recordargs (fn _ => let val  (ID as ID1) = ID1
 ()
 val  (exp as exp1) = exp1 ()
 in ((Symbol.symbol ID, exp, EQright) :: nil)
end)
 in ( LrTable.NT 11, ( result, ID1left, exp1right), rest671)
end
|  ( 25, ( ( _, ( MlyValue.exp exp1, _, exp1right)) :: ( _, ( _, _, 
EQright)) :: ( _, ( MlyValue.ID ID1, _, _)) :: _ :: ( _, ( 
MlyValue.recordargs recordargs1, recordargs1left, _)) :: rest671)) =>
 let val  result = MlyValue.recordargs (fn _ => let val  (recordargs
 as recordargs1) = recordargs1 ()
 val  (ID as ID1) = ID1 ()
 val  (exp as exp1) = exp1 ()
 in ((Symbol.symbol ID, exp, EQright) :: recordargs)
end)
 in ( LrTable.NT 11, ( result, recordargs1left, exp1right), rest671)

end
|  ( 26, ( ( _, ( _, _, RPAREN1right)) :: ( _, ( MlyValue.explist 
explist1, _, _)) :: _ :: ( _, ( MlyValue.ID ID1, ID1left, IDright)) ::
 rest671)) => let val  result = MlyValue.recordexp (fn _ => let val  (
ID as ID1) = ID1 ()
 val  (explist as explist1) = explist1 ()
 in (
A.CallExp { func = Symbol.symbol ID, args = explist, pos = IDright })

end)
 in ( LrTable.NT 12, ( result, ID1left, RPAREN1right), rest671)
end
|  ( 27, ( ( _, ( _, _, RBRACE1right)) :: ( _, ( MlyValue.recordargs 
recordargs1, _, _)) :: _ :: ( _, ( MlyValue.ID ID1, ID1left, IDright))
 :: rest671)) => let val  result = MlyValue.recordexp (fn _ => let
 val  (ID as ID1) = ID1 ()
 val  (recordargs as recordargs1) = recordargs1 ()
 in (
A.RecordExp { fields = recordargs, typ = Symbol.symbol ID, pos = IDright }
)
end)
 in ( LrTable.NT 12, ( result, ID1left, RBRACE1right), rest671)
end
|  ( 28, ( ( _, ( _, _, NIL1right)) :: ( _, ( MlyValue.ID ID1, ID1left
, IDright)) :: rest671)) => let val  result = MlyValue.recordexp (fn _
 => let val  (ID as ID1) = ID1 ()
 in (
A.RecordExp { fields = [], typ = Symbol.symbol ID, pos = IDright })
...
MlyValue.exp (fn _ => let val  (exp as exp1) = exp1 ()
 in (
A.OpExp {left = A.IntExp 0, oper = A.MinusOp, right = exp, pos = MINUSright}
)
end)
...
|  ( 33, ( ( _, ( MlyValue.STRING STRING1, STRING1left, (STRINGright
...
|  ( 35, ( ( _, ( MlyValue.recordexp recordexp1, recordexp1left, 
recordexp1right)) :: rest671)) => let val  result = MlyValue.exp (fn _
 => let val  (recordexp as recordexp1) = recordexp1 ()
 in (recordexp)
end)
 in ( LrTable.NT 8, ( result, recordexp1left, recordexp1right), 
rest671)
end
...
|  ( 49, ( ( _, ( MlyValue.lvalue_not_id lvalue_not_id1, 
...
lvalue_not_id1) = lvalue_not_id1 ()
 ...
 in ( LrTable.NT 8, ( result, lvalue_not_id1left, lvalue_not_id1right)
, rest671)
end
...
MlyValue.exp exp1, _, _)) :: ( _, ( _, IF1left, IFright)) :: rest671))
 ...
 val  exp2 = exp2 ()
 in (
A.IfExp { test = exp1, then' = exp2, else' = NONE, pos = IFright })
...
MlyValue.exp exp2, _, _)) :: _ :: ( _, ( MlyValue.exp exp1, _, _)) ::
 _ :: ( _, ( MlyValue.ID ID1, _, _)) :: ( _, ( _, FOR1left, FORright))
 ...
 val  exp3 = exp3 ()
 in (
A.ForExp { var = Symbol.symbol ID, lo = exp1, hi = exp2, body = exp3, pos = FORright, escape = ref true }
)
end)
 in ( LrTable.NT 8, ( result, FOR1left, exp3right), rest671)
end
|  ( 55, ( ( _, ( _, _, END1right)) :: ( _, ( MlyValue.optionalexp 
optionalexp1, _, _)) :: _ :: ( _, ( MlyValue.decs decs1, _, _)) :: ( _
, ( _, LET1left, LETright)) :: rest671)) => let val  result = 
MlyValue.exp (fn _ => let val  (decs as decs1) = decs1 ()
 val  (optionalexp as optionalexp1) = optionalexp1 ()
 in (A.LetExp { decs = rev decs, body = optionalexp, pos = LETright })

end)
 in ( LrTable.NT 8, ( result, LET1left, END1right), rest671)
end
|  ( 56, ( ( _, ( _, BREAK1left, (BREAKright as BREAK1right))) :: 
...
|  ( 58, ( ( _, ( MlyValue.exp exp2, exp2left, exp2right)) :: _ :: ( _
...
|  ( 60, ( ( _, ( MlyValue.lvalue_not_id lvalue_not_id1, 
lvalue_not_id1left, lvalue_not_id1right)) :: rest671)) => let val  
...
|  ( 62, ( ( _, ( _, _, (RBRACKright as RBRACK1right))) :: ( _, ( 
...
|  ( 63, ( ( _, ( _, _, (RBRACKright as RBRACK1right))) :: ( _, ( 
...
lvalue_not_id1) = lvalue_not_id1 ()
 ...
 in ( LrTable.NT 14, ( result, lvalue_not_id1left, RBRACK1right), 
rest671)
end
